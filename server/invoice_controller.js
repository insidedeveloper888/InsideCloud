/**
 * Invoice Controller
 * Handles all business logic for invoice and payment management
 * Routes for: invoices, invoice_items, invoice_payments, invoice_settings
 */
 ...
 */
async function getInvoiceSettings(ctx) {
  try {
 ...
 */
async function updateInvoiceSettings(ctx) {
  try {
    const { organization_slug } = ctx.query;
...
/**
 * POST /api/invoice_settings/preview
 */
async function previewInvoiceFormat(ctx) {
  try {
...
async function getInvoices(ctx) {
  try {
    const { organization_slug, status, customer_id, sales_order_id, sales_person_id, date_from, date_to, overdue } = ctx.query;
 ...
 */
async function getInvoicePayments(ctx) {
  try {
    const { id } = ctx.params;
    const { organization_slug } = ctx.query;
 ...
 */
async function addInvoicePayment(ctx) {
  try {
    const { id } = ctx.params;
    const { organization_slug } = ctx.query;
    ...
    if (!invoice) {
      return (ctx.status = 404), (ctx.body = { error: 'Invoice not found' });
    }
 ...
 */
async function deleteInvoicePayment(ctx) {
  try {
    ...
    if (!payment) {
      return (ctx.status = 404), (ctx.body = { error: 'Payment not found' });
    }
...
// ============================================================================
// INVOICE STATUS CONFIGURATION ENDPOINTS
// ============================================================================
 ...
 */
async function getInvoiceStatuses(ctx) {
  try {
    const { organization_slug } = ctx.query;
 ...
 */
async function updateInvoiceStatuses(ctx) {
  try {
    const { organization_slug } = ctx.query;
