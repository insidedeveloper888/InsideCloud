...
const DashboardTab = ({ projects, statuses = [] }) => {
    ...
    return (
        <div className="space-y-6 md:space-y-8 px-4 md:px-0">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-purple-600 to-blue-600 shadow-lg text-white p-6 md:p-8">
                <div className="relative z-10">
                    <h2 className="text-xs md:text-sm font-medium text-purple-100 uppercase tracking-wider mb-2">Total Project Value</h2>
                    <div className="text-3xl md:text-5xl font-bold mb-2">
                        ${totalValue.toLocaleString()}
                        <span className="text-sm md:text-lg font-normal text-purple-200 ml-2">Estimated</span>
                    </div>
                    <p className="text-sm md:text-base text-purple-100 opacity-80">Based on active project budgets</p>
                </div>
                {/* Decorative Circle */}
                    ...
                    <div className="space-y-6">
                        {projects.length === 0 ? (
                            ...
                            projects.slice(0, 5).map((project) => {
                                const creatorName = project.created_by?.display_name || project.owner?.display_name || 'Unknown User';
                                ...
                                <div key={project.id} className="flex items-start space-x-4">
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm text-gray-900">
                                            <span className="font-medium">{creatorName}</span> created project <span className="font-medium text-blue-600">{project.name}</span>
                                            {project.status && (
                                                <span
