...
import { FileText, X, CheckCircle, Truck, Eye, Upload, Trash2 } from 'lucide-react';
import { getPOStatusColor, getPOStatusLabel, getPOStatusIcon } from '../../utils/helpers';
import { InventoryAPI } from '../../api/inventory';

/**
 * Purchase Order Detail Modal
 * Shows full PO details with ability to:
 * - Update status
...
export default function PODetailModal({
  ...
  const handleUploadDO = async (file) => {
    try {
      setLocalError('');
      await onUploadDO(file);
    } catch (err) {
      console.error('Failed to upload DO:', err);
      setLocalError(`Failed to upload delivery order: ${err.message}`);
    }
  };
  ...
  const handleDelete = async () => {
    if (window.confirm(`⚠️ Delete Purchase Order "${selectedPO.po_number}"?\n\nThis action cannot be undone. The purchase order and all its items will be permanently removed.`)) {
      try {
        ...
        setLocalError(`Failed to delete purchase order: ${err.message}`);
      }
    }
  };
  ...
  const handleUpdateExpectedDate = async (newDate) => {
    try {
      setLocalError('');
      ...
      setLocalError(`Failed to update expected delivery date: ${err.message}`);
    }
  };
            ...
            <div>
              ...
              {selectedPO.status !== 'received' ? (
                <input
                  ...
                  onChange={(e) => handleUpdateExpectedDate(e.target.value)}
