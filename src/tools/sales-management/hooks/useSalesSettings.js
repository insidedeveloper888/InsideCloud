...
export function useSalesSettings(organizationSlug) {
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${resolveApiOrigin()}/api/sales_order_settings?organization_slug=${organizationSlug}`,
        { credentials: 'include' }
      );
  ...
  const previewFormat = useCallback(async (format, sampleCounter = 1) => {
    try {
      const response = await fetch(
        `${resolveApiOrigin()}/api/sales_order_settings/preview_format`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
